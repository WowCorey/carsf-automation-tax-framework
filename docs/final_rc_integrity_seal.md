...
Build 25 sealed the previous RC state. Build 26 adds real-data feasibility and calibration-intake artefacts after that state, Build 27 adds public aggregate-data pilot and realistic-placeholder anchor artefacts after that state, and Build 28 adds a public-data reviewer evidence map after that state. A new integrity seal must be regenerated if Builds 26-28 are included in a later sealed RC.
