...
def test_public_data_evidence_map_dashboard_reads_source_locator_pack(repo_root) -> None:
    page = repo_root / "simulator" / "pages" / "29_Public_Data_Evidence_Map.py"
    text = page.read_text(encoding="utf-8")

    assert "source_locator_verification_pack.json" in text
    assert "Source-Locator Verification Pack" in text
    assert "requests" not in text
    assert "urllib" not in text


def test_public_data_evidence_map_dashboard_has_no_score_widgets_after_source_locator_update(repo_root) -> None:
    page = repo_root / "simulator" / "pages" / "29_Public_Data_Evidence_Map.py"
...
def test_public_data_evidence_map_streamlit_page_imports_after_source_locator_update(repo_root) -> None:
    runpy.run_path(str(repo_root / "simulator" / "pages" / "29_Public_Data_Evidence_Map.py"), run_name="__main__")


def test_main_streamlit_app_imports_after_source_locator_update(repo_root) -> None:
