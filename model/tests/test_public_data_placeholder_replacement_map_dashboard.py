...
def test_public_data_evidence_map_dashboard_reads_placeholder_replacement_map(repo_root) -> None:
    page = repo_root / "simulator" / "pages" / "29_Public_Data_Evidence_Map.py"
    text = page.read_text(encoding="utf-8")

    assert "public_data_placeholder_replacement_map.json" in text
    assert "Public Data Placeholder Replacement Map" in text
    assert "requests" not in text
    assert "urllib" not in text


def test_public_data_evidence_map_dashboard_has_no_scores_after_placeholder_replacement_map(repo_root) -> None:
    page = repo_root / "simulator" / "pages" / "29_Public_Data_Evidence_Map.py"
    ...
    assert "liability estimate" not in lowered


def test_public_data_evidence_map_streamlit_page_imports_after_placeholder_replacement_map(repo_root) -> None:
    runpy.run_path(str(repo_root / "simulator" / "pages" / "29_Public_Data_Evidence_Map.py"), run_name="__main__")


def test_main_streamlit_app_imports_after_placeholder_replacement_map(repo_root) -> None:
